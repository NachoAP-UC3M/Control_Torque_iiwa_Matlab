...
%%%% 2nd step: calculate the hidden layer using the Transfer fucntion radbas (Normalizaed radial basis)
%%%% transfer function )
%H=radbas(input_weights*X');
H=logsig(input_weights*X');
...
Y_hat=(H' * B) ;
Yts_hat=(logsig(input_weights*Xts')'*B);
